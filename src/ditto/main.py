...
os.chdir("/oscar/data/gk/voommen/MATCHO/TimeOperator/src/ditto")
print(os.getcwd() )
...
def train(configs, logger):
    for model_name in configs.models:
        ...
        if configs.train[model_name]:
            ...
            wandb.init(project="DiTTO-Revision",
                       config=configs,
                       group=group,
                       name=model_name)
...
def test(configs, logger, zero_shot_noise=False):
    final_metrics = {"l2_loss": {}, "l2_abs_loss": {}, "l_inf_loss": {}, "l_inf_loss_abs": {}, "mse_loss": {}, "mae_loss": {}, "rmse_loss": {}, "r2_score": {}}
    for inference_steps in configs.time_steps_inference_list:
        ...
        for model_name in configs.models:
            ...
            for error_type in final_metrics.keys():
                key = list(metric[error_type].keys())[0]
                val = list(metric[error_type].values())[0]
                val_key = list(val.keys())[0]

                if key not in final_metrics[error_type]:
                    final_metrics[error_type][key] = {}
                final_metrics[error_type][key][val_key] = val[val_key]
    ...
    for error_type in final_metrics.keys():
        metrics = save_metrics_to_text(final_metrics[error_type],
                                       path_model=tester.path_model,
                                       noise=configs.noise_level,
...
def ns_20():
    ...
    configs.models = [
        'ditto',
        'ditto_point',
        f'ditto_subsampled_X{configs.subsample}',
        f'ditto_point_subsampled_X{configs.subsample}_gate',
        f'ditto_point_subsampled_X{configs.subsample}',
        'FNO',
        'UNet',
        'RNN',
    ]
    configs.train = {'ditto': False,
                    'ditto_point': False,
                    f'ditto_subsampled_X{configs.subsample}': False,
                    f'ditto_point_subsampled_X{configs.subsample}': False,
                    f'ditto_point_subsampled_X{configs.subsample}_gate': False,
                    'FNO': False,
                    'UNet': False,
                    'RNN': False}
    return configs
...
def ns_2007_large():
    ...
    configs.train = {f'ditto_subsampled_X{configs.subsample}': True,
                    f'ditto_point_subsampled_X{configs.subsample}': True,
                    f'ditto_point_subsampled_X{configs.subsample}_gate': True,
                    'FNO': True,
                    'RNN': False}
    return configs
...
CONFIG_RUNS = [burgers_1, burgers_2, burgers_3,
               ns_20, ns_2007, ns_2007_large,
               wave2d, hypersonics, hypersonics_time,
               climate_2d_level_10, climate_2d_level_1000,
               wave3d, climate_3d, shallow_water]


if __name__ == "__main__":
    logger = get_logger()

    configs = CONFIG_RUNS[RUN_ID]()
